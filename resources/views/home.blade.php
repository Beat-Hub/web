                        ...
                        <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
                            <a class="block relative h-48 rounded overflow-hidden">
                                @foreach ($users as $user)
                                    <img alt="{{ $beat->beat_name }}" class="object-cover object-center w-full h-full block" src="/images/{{ $user->image}}">
                                @endforeach
                            </a>
                            <div class="mt-4">
                                <h3 class="text-red-500 uppercase text-xs tracking-widest title-font mb-1">{{ $beat->genre  }}</h3>
                                <h2 class="text-red-900 title-font capitalize text-lg font-medium">{{ $beat->beat_name }}</h2>
